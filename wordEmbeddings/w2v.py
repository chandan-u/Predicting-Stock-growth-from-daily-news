...
def getAvgDocVectors(w2v, train_X, test_X):
    ...
    for index, sentence_tokens in enumerate(train_sentences_tokens):
        ...
        train_docvectors[index] = np.mean([w2v[word]  for word in sentence_tokens if word in w2v_words])

    for index, sentence_tokens in enumerate(test_sentences_tokens):

        test_docvectors[index] = np.mean([w2v[word]  for word in sentence_tokens if word in w2v_words])
